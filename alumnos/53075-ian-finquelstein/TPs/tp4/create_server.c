...
void create_server(int port_value) 
{
    ...
    int socket_connect;
    int bytes_read;
    int val;//Variable necesaria para el seteo de opc. del socket
    ...
    signal(SIGCHLD,SIG_IGN);//Padre ignora señales del hijo para no bloquearse.
    //Terminado el hijo se lo saca de la tabla de procesos para que no queden zombies.

    while((socket_connect = accept(socket_listen,NULL,0)) > 0){//Asigno un nuevo socket a cada conexion entrante
            if(fork() == 0){

                    bytes_read = read(socket_connect, buffer, sizeof buffer);//Leo del socket hijo para recibir la peticion
                    ...
                    if (check_get_method != 0)//Si metodo != GET -> error sino muestro el recurso pedido
                    {
                        ...
                        write(socket_connect, buffer, strlen(buffer));//Tener cuidado entre sizeof y strlen cuando escribo al cliente la cabecera http
                        
                        while((bytes_read = read(fd_error, buffer, sizeof buffer)) > 0)//Escribo en el socket el html
                            write(socket_connect, buffer, bytes_read);
